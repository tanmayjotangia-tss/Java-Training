package com.tss.day26.Observer.app;
...
import com.tss.day26.Observer.model.EmailNotifier;
import com.tss.day26.Observer.model.SMSNotifier;
import com.tss.day26.Observer.model.WhatsAppNotifier;
import com.tss.day26.Observer.service.CurrentAccount;
import com.tss.day26.Observer.service.SavingAccount;
        ...
        while (true) {
            ...
            switch (choice) {
                case 1:
                    ...
                    } while (InputUtil.confirmAction("Do you want to add more account ? "));
                    break;
...
//                receiver.addTransaction(recieverTransaction);
    ...
    private static void addObserver() {
        Account account = selectAccount();
        if (account == null) {
            System.out.println("No account selected to add observers.");
            return;
        }

        while (true) {
            System.out.println("\nChoose observer to add:");
            ...
            switch (choice) {
                case 1 -> {
                    String smsNumber = InputUtil.readString("Enter phone number for SMS notifications: ");
                    account.addObserver(new SMSNotifier(smsNumber));
                    System.out.println("SMS Observer added.");
                }
                case 2 -> {
                    String email = InputUtil.readEmail("Enter email for Email notifications: ");
                    account.addObserver(new EmailNotifier(email));
                    System.out.println("Email Observer added.");
                }
                case 3 -> {
                    String whatsappNumber = InputUtil.readString("Enter phone number for WhatsApp notifications: ");
                    account.addObserver(new WhatsAppNotifier(whatsappNumber));
                    System.out.println("WhatsApp Observer added.");
                }
                case 4 -> {
                    System.out.println("Finished adding observers.");
                    return;
                }
